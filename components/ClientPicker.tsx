...
export default function ClientPicker({
  ...
  const filtered = useMemo(() => {
    ...
    const candidates = clinicId ? clients.filter((client) => client.clinic_id === clinicId) : clients;
    ...
    return candidates
      ...
      .slice(0, 8);
  }, [clients, clinicId, query]);

  const selected = clients.find((c) => c.id === selectedId) ?? null;

  const pick = (c: Client) => {
    const clinic = clinics.find((item) => item.id === c.clinic_id);
    onSelect(c, {
      naam_patient: c.naam_patient ?? "",
      geboortedatum: c.geboortedatum ?? "",
      behandelaar: clinic?.behandelaar ?? c.behandelaar ?? "",
      klant_regel2: clinic?.address ?? c.klant_regel2 ?? "",
      // El nombre de la clínica alimenta el bloque "Kliniek / adres" del PDF.
      // Solo se conserva el encargo antiguo cuando el paciente no tiene clínica.
      in_opdracht: clinic?.name ?? c.in_opdracht ?? "",
    });
    setQuery("");
    setOpen(false);
    setActiveIndex(-1);
    ...
    <div className="relative">
      ...
      {selected ? (
        <div className="flex items-center justify-between rounded-lg border border-[var(--line)] bg-[var(--tint)] px-3 py-2 text-sm">
          <span className="font-medium text-[var(--navy)]">{selected.naam_patient || selected.behandelaar || "(sin nombre)"}</span>
          <button
            type="button"
      ...
      {open && !selected && filtered.length > 0 && (
        ...
        >
          {filtered.map((c, idx) => (
            ...
            >
              <div className="font-medium text-[var(--navy)]">{c.naam_patient || c.behandelaar || "(sin nombre)"}</div>
              <div className="text-xs text-[var(--muted)]">{clinics.find((clinic) => clinic.id === c.clinic_id)?.name || "Sin clínica asignada"}</div>
            </div>
          ))}
        </div>
      )}
